...
int main() {
    ...
    if (!(fecha1.dia >= 1 && fecha1.dia <= 31 && fecha1.mes >= 1 && fecha1.mes <= 12)) {
            printf("Invalid input for the first date\n");
            return 1;
    }

    printf("Introduce otro día, mes y año separados por espacios: ");
    scanf("%d %d %d", &fecha2.dia, &fecha2.mes, &fecha2.anyo);
    if (!(fecha2.dia >= 1 && fecha2.dia <= 31 && fecha2.mes >= 1 && fecha2.mes <= 12)) {
                printf("Invalid input for the second date\n");
                return 1;
    }
